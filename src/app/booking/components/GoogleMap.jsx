...
export default function GoogleMap() {
	return (
		// ВЫЕЗЖАЕТ СНИЗУ ПРИ СКРОЛЛЕ
		<FadeInFromBottom delay={100} threshold={0.3}>
			{/* === ГУГЛ КАРТА ИМПОРТИРОВАННАЯ С ГУГЛ КАРТ === */}
			<div className='flex items-center justify-center pt-50'>
				<div className='w-[95%] h-130 rounded-xl overflow-hidden shadow-3xl border-3 border-black'>
					<div className='w-full h-full'>
						{/* GOOGLE MAPS ВСТРАИВАЕТСЯ ЧЕРЕЗ iframe
								ССЫЛКА ПОЛУЧЕНА GOOGLE КАРТЫ - ПОДЕЛИТЬСЯ - ВСТРАИВАНИЕ КАРТЫ - КОПИРОВАТЬ SRC
								ПАРАМЕТРЫ iframe =
								allowFullScreen - РАЗРЕШАЕТ ПОЛНОЭКРАННЫЙ РЕЖИМ
								loading='lazy' - ЗАГРУЖАЕТ ТОЛЬКО ТОГДА КОГДА ВИДНА (УСКОРЯЕТ СТРАНИЦУ)
								referrerPolicy - ЗАЩИТА ОТ УТЕЧЕК ДАННЫХ
